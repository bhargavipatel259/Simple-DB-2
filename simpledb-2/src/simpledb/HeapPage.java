// Took reference from Blackboard pa1 jar file.
...
public class HeapPage implements Page {
	...
	public Tuple getTuple(int entryID) {
		// some code goes here
		/**
		 * Here first it checks the length of data,
		 * if it is greater than equal to zero it will fetch ID of entry stored in tuple.
		 */
		//Tuple tup = new Tuple(td);
		/*if((data.length) >= 0)
		{
			tup = getTuple(entryID);
		}
		else
		{
			return null;
		...
		createTuple(new DataInputStream(new ByteArrayInputStream(data, entryID, data.length - entryID)));
	...
	public Iterator<Tuple> iterator() {
		// some code goes here
		/**
		 * If The start_ptr is less than entryCount it will return true,
		 * else will return false.Then using hasNext method we find and return the value of t.
		 */
	/*	return new Iterator<Tuple>() {
			private int start_ptr = 0;
			
				public boolean hasNext() {
					if(start_ptr < entryCount()) 
					{
						return true;
					}
						return false;
...
*/
	...
	/*	RecordId rid = t.getRecordId();
		...
		t.getRecordId().tupleno();
		//t.saveTupleLocation(entryID,location);
	*/
		RecordId rid = t.getRecordId();
	...
	public void addTuple(Tuple t) throws DbException {
		...
		{
			throw new DbException("error while inserting");
		}
			int location = endOfFreeSpace() - b.length;
			System.arraycopy(b, 0, data, location, b.length);
			this.saveEntryCount(count);
			count++;
			saveTupleLocation(entryCount(),location);
			RecordId rid = new RecordId(pid,entryCount());
			t.setRecordId(rid);
		*/
		byte[] b = toByteArray(t);
	    if (freeSpaceSize() < b.length + 4) {
